...
public class ServerNoSplit {
    ...
    public static void main(String[] args) {
        ...
        new ServerNoSplit("SERVER",null,new ArrayList<>());

    }

    private ServerNoSplit(String serverName, Coordinate centerPoint, List<ObjectId> postsList)
    {
        if(centerPoint!=null)
        {
        ...
        try {
            ...
            connection.setClientID("NOSPLITSERVER");
    ...
    private void serverSplit(Coordinate coordinate) throws JMSException {
        ...
        System.out.println("SERVER SPLIT OCCURRED, branching around coords: "+coordinate.getRadius()+" into :"+newServerName);
        List<ObjectId> postlist=new ArrayList<>();
        coordinate.setRadius(coordinate.getRadius()*1.5);
        for (ObjectId post: posts)
        {
            DBObject DBpost= getDbObject(post);
            DBObject coords=(DBObject)DBpost.get("coordinate");
        ...
        new ServerNoSplit(newServerName,coordinate,postlist);
    }
}
